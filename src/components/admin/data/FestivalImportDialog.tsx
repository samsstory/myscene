    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) resetState(); onOpenChange(v); }}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Paste JSON or CSV data to bulk-import festival lineups.</DialogDescription>
        </DialogHeader>

        <Tabs value={mode} onValueChange={(v) => { setMode(v as "json" | "csv"); resetState(); }}>
          <TabsList className="mb-3 gap-1 rounded-lg bg-white/[0.03] border border-white/[0.06] p-1 h-auto">
            <TabsTrigger value="json" className="rounded-md px-3 py-1.5 text-xs font-medium text-muted-foreground data-[state=active]:bg-white/[0.07] data-[state=active]:text-foreground">
              JSON
            </TabsTrigger>
            <TabsTrigger value="csv" className="rounded-md px-3 py-1.5 text-xs font-medium text-muted-foreground data-[state=active]:bg-white/[0.07] data-[state=active]:text-foreground">
              CSV
            </TabsTrigger>
          </TabsList>
              ...
              {parsed.map((f, i) => (
                <div key={i} className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    {f.venue_name && (
                      <p className="text-xs text-muted-foreground truncate">{f.venue_name}</p>
                    )}
                  </div>
