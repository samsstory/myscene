...
import { X, UserPlus, Users, Search, Loader2, UserCheck, Calendar, Star, Zap, Music2 } from "lucide-react";
...
export default function FriendsPanel({ open, onOpenChange }: FriendsPanelProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="bottom"
        className="h-[92dvh] flex flex-col rounded-t-3xl px-0 bg-background/90 backdrop-blur-2xl border-t border-white/[0.10] overflow-hidden"
      >
        {/* Drag handle */}
        ...
        <div className="flex items-center justify-between px-5 pb-4 flex-shrink-0">
          ...
          <button
            ...
            className="w-8 h-8 rounded-full bg-white/[0.06] border border-white/10 flex items-center justify-center hover:bg-white/[0.10] transition-colors"
          >
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto px-5 pb-8 space-y-2.5">
              {isLoading ? (
                ...
                /* Following but no activity matching filter */
